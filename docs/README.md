# Documentation Serenity

Une page par phase, dans l'ordre de construction.

| Page | Sujet |
|---|---|
| [00 — Vue d'ensemble](00-vue-ensemble.md) | Le projet, l'architecture, les principes |
| [01 — Dépôt et GitHub](01-depot-github.md) | Arborescence, CI, labels, protection de `main` |
| [02 — Infrastructure](02-infrastructure.md) | Docker Compose, Vaultwarden, ntfy, Tailscale |
| [03 — Socle backend](03-backend-socle.md) | FastAPI, modèles, authentification, audit |
| [04 — Coffre](04-coffre.md) | Bitwarden CLI, compte dédié, synchronisation |
| 05 — Veille | Fuites, k-anonymat, mots de passe faibles *(à venir)* |
| 06 — Agent | Planification, kill switch, notifications *(à venir)* |
| 07 — Interface | PWA React *(à venir)* |
| 08 — Sauvegardes | restic, restauration *(à venir)* |
| 09 — Sécurité | Modèle de menace, risques restants *(à venir)* |

Références :

- `api.md` : l'API REST *(phase 6)*
- `design.md` : la charte graphique *(phase 7)*
- [`decisions/`](decisions/README.md) : les décisions techniques (ADR)
